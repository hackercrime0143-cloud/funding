...
async function seedDB() {
  try {
    ...
    // Drop the conflicting stale index 'userId_1' if it exists in 'bankdetails'
    try {
      const collection = mongoose.connection.db.collection('bankdetails');
      const indexes = await collection.indexes();
      const hasStaleIndex = indexes.some(idx => idx.name === 'userId_1');
      if (hasStaleIndex) {
        console.log('Dropping stale unique index "userId_1" from bankdetails...');
        await collection.dropIndex('userId_1');
        console.log('Stale unique index "userId_1" dropped successfully!');
      }
    } catch (idxErr) {
      console.error('Error checking or dropping index "userId_1" in bankdetails:', idxErr);
    }

    // Update/migrate existing VirtualAccount documents with default values
    try {
      await VirtualAccount.updateMany(
        { is_enabled: { $exists: false } },
        { $set: { is_enabled: true, allow_concurrent: false, qr_code: '', last_assigned_at: new Date() } }
      );
      console.log('VirtualAccount documents migrated successfully!');
    } catch (migErr) {
      console.error('Error migrating VirtualAccount fields:', migErr);
    }
...
let expiryIntervalStarted = false;
async function startExpiryInterval() {
  if (expiryIntervalStarted) return;
  expiryIntervalStarted = true;
  
  setInterval(async () => {
    try {
      const { Order, VirtualAccount } = await import('./models');
      ...
      if (ordersToExpire.length > 0) {
        const orderIds = ordersToExpire.map(o => o._id);
        const vaIds = ordersToExpire.map(o => o.virtual_account_id).filter(Boolean);
      ...
      console.error('Failed to run automatic payment expiry:', e);
    }
