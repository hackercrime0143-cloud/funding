...
const OrderSchema = new mongoose.Schema({
  ...
  qr_status: { type: String, enum: ['pending', 'paid', 'expired', 'cancelled'], default: 'pending' },
  qr_expiry_at: { type: Date },
  qr_paid_at: { type: Date, default: null }
});
...
UserSchema.index({ role: 1, is_suspended: 1, created_at: -1 });
UserSchema.index({ role: 1, wallet_balance: -1 });
