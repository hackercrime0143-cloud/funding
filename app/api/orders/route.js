...
export async function POST(request) {
  try {
    ...
    if (paymentMethod === 'wallet') {
      const buyer = await User.findById(session.id);
      if (!buyer) {
        return NextResponse.json({ error: 'User not found.' }, { status: 404 });
      }
      if (buyer.wallet_balance < scheme.price) {
        return NextResponse.json({ error: 'Insufficient wallet balance for this purchase.' }, { status: 400 });
      }

      // Delete any draft order for this user & scheme to prevent orphaned drafts
      await Order.deleteMany({
        user_id: session.id,
        scheme_id: scheme._id || null,
        status: 'pending',
        utr: { $regex: /^DRAFT-/ }
      });

      // Deduct wallet balance
      buyer.wallet_balance -= scheme.price;
      await buyer.save();
      ...
      const walletTx = await Transaction.create({
        ...
        amount: -scheme.price,
        status: 'completed',
        utr: `WALLET-${Date.now()}-${Math.floor(1000 + Math.random() * 9000)}`
      });

      // Payout first day yield immediately
      const dailyYield = scheme.price * scheme.daily_return_rate;
      buyer.wallet_balance += (scheme.price + dailyYield);
      await buyer.save();

      // Create scheme purchase order immediately active
      const newOrder = await Order.create({
        ...
        days_remaining: Math.max(0, scheme.days - 1),
        status: 'active',
        utr: walletTx.utr,
      ...
      // Link order to wallet transaction
      walletTx.order_id = newOrder._id;
      await walletTx.save();

      // Log scheme yield payout
      ...
      if (buyer.referred_by_id) {
        ...
        if (levelA) {
          const commL1 = scheme.price * 0.003;
          ...
          if (levelA.referred_by_id) {
            ...
            if (levelB) {
              const commL2 = scheme.price * 0.0015;
      ...
      if (expiredOrder) {
        ...
        if (expiredUser) {
          expiredUser.wallet_balance += scheme.price;
      ...
      // Notify admin
      await createAdminNotification(session.id, scheme.price, `Purchased scheme (Wallet)`);

      return NextResponse.json({
        success: true,
        message: `Scheme purchased successfully using wallet balance.${matchedMessage}`,
